(() => {
  ...
  function filteredTransactionsForMonth(month) {
    ...
    return (state.data.transactions || [])
      ...
      .filter((row) => !person || [row.paid_by, row.owner_user, row.other_person].includes(person))
  ...
  function activeFixedCostsForMonth(month) {
    return (state.data.fixedCosts || [])
      ...
      .filter((row) => {
        const isActive = String(row.active || "").toLowerCase();
        if (isActive && isActive !== "ja") return false;
  ...
  function calculatePeerBalance(rows) {
    ...
    rows.forEach((row) => {
      ...
      if (row.split_percent !== undefined && row.split_percent !== "") {
        const splitPercent = Number(row.split_percent || 50);
        myShare = amount * (splitPercent / 100);
        otherShare = amount - myShare;
      } else {
        const splitMax = Number(row.split_max || 50) / 100;
        const splitOther = Number(row.split_other || 50) / 100;
        myShare = amount * splitMax;
        otherShare = amount * splitOther;
      }
  ...
  function renderFixedCostsTable() {
    ...
    const rows = (state.data.fixedCosts || []).filter(isVisibleFixedCost);
  ...
  function renderIncomeTable() {
    ...
    const rows = (state.data.income || []).filter(isVisibleIncome);
