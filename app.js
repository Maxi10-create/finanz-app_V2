(() => {
  ...
  function renderVacationLabels(data) {
    setText(els.vacationSectionTitle, "Urlaubsanalyse");
    setText(els.vacationSectionSubtitle, "Kennzahlen und Kostenaufteilung nach Jahr und Urlaub");

    if (data.isYearSelection) {
      setText(els.vacationTotalCostLabel, "Eigene Ausgaben Auswahl");
      setText(els.vacationTotalCostSub, "Eigener Anteil über alle Urlaube im Jahr");
      setText(els.vacationTripCountLabel, "Reisen in Auswahl");
      setText(els.vacationTripCountSub, `${data.relevantTrips.length} Reisen`);

      setText(els.vacationKpiTitle, "Kostenaufteilung nach Kategorie");
      setText(els.vacationKpiSubtitle, "Eigener Anteil für die aktuelle Auswahl");
      setText(els.vacationChartTitle, "Kategorien der aktuellen Auswahl");
      setText(els.vacationChartSubtitle, "Eigener Anteil in €");
      return;
    }

    if (data.hasSharedTrip) {
      setText(els.vacationTotalCostLabel, "Gesamtausgaben Urlaub");
      setText(els.vacationTotalCostSub, "Gesamt beider Nutzer");
      setText(els.vacationTripCountLabel, "Ausgaben pro Person");
      setText(els.vacationTripCountSub, `${PERSON_A} / ${PERSON_B}`);

      setText(els.vacationKpiTitle, "Kostenaufteilung nach Kategorie");
      setText(els.vacationKpiSubtitle, "Gesamt und pro Person für den ausgewählten Urlaub");
      setText(els.vacationChartTitle, "Kategorien des ausgewählten Urlaubs");
      setText(els.vacationChartSubtitle, "Gesamt sowie pro Person in €");
      return;
    }

    setText(els.vacationTotalCostLabel, "Eigene Ausgaben Urlaub");
    setText(els.vacationTotalCostSub, "Eigener Anteil im ausgewählten Urlaub");
    setText(els.vacationTripCountLabel, "Ausgewählte Reisen");
    setText(els.vacationTripCountSub, `${data.relevantTrips.length} Reise`);

    setText(els.vacationKpiTitle, "Kostenaufteilung nach Kategorie");
    setText(els.vacationKpiSubtitle, "Eigener Anteil für den ausgewählten Urlaub");
    setText(els.vacationChartTitle, "Kategorien des ausgewählten Urlaubs");
    setText(els.vacationChartSubtitle, "Eigener Anteil in €");
  }
  ...
  function renderVacationOverview() {
    ...
    if (els.vacationTotalCost) {
      const mainValue = data.isYearSelection
        ? data.currentUserTotal
        : (data.hasSharedTrip ? data.totals.total : data.currentUserTotal);
